...
class AdminController extends Controller
{
    public function editHistory(Request $request): JsonResponse
    {
        ...
        // Collect all post edit history
        $postHistory = PostEditHistory::with('editor:id,username,avatar_url')->get();
        $postIds = $postHistory->pluck('post_id')->unique();
        ...
        $postMapped = $postHistory->map(fn ($h) => [
            'id'          => $h->id,
            'type'        => 'post',
        ...
        // Collect all comment edit history
        $commentHistory = CommentEditHistory::with('editor:id,username,avatar_url')->get();
        $commentIds = $commentHistory->pluck('comment_id')->unique();
        ...
        $commentMapped = $commentHistory->map(fn ($h) => [
            'id'          => $h->id,
            'type'        => 'comment',
        ...
        $merged = collect($postMapped)
            ->concat($commentMapped)
