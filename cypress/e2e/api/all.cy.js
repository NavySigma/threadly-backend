describe("Threadly API — Complete Test Suite", () => {
    ...
    it("Setup: give poster 15 reputation points", () => {
        cy.task("php", {
            command: "cypress/scripts/set_points.php poster 15",
        }).then((out) => {
    ...
    it("Setup: login superadmin", () => {
        cy.request({
            method: "POST",
            url: "/login",
            body: { email: USERS.superadmin.email, password: USERS.superadmin.password },
        }).then((res) => {
    ...
    it("Setup: login poster", () => {
        cy.apiLogin(TEST.poster.email, TEST.poster.password).then((res) => {
            expect(res.access_token).to.exist;
        });
    });

    it("Setup: login commenter", () => {
        cy.apiLogin(TEST.commenter.email, TEST.commenter.password).then(
            (res) => {
                expect(res.access_token).to.exist;
            },
        );
    ...
    it("Setup: login voter", () => {
        cy.apiLogin(TEST.voter.email, TEST.voter.password).then((res) => {
            expect(res.access_token).to.exist;
        });
    });

    it("Setup: login other", () => {
        cy.apiLogin(TEST.other.email, TEST.other.password).then((res) => {
            expect(res.access_token).to.exist;
        });
    });
    ...
    context("Auth", () => {
        ...
        it("login: wrong password returns 401", () => {
            cy.request({
                method: "POST",
                url: "/login",
                body: { email: TEST.poster.email, password: "wrongpass" },
                failOnStatusCode: false,
            }).then((res) => {
        ...
        it("login: re-login after logout works", () => {
            cy.apiLogin(TEST.poster.email, TEST.poster.password).then(
                (res) => {
                    expect(res.access_token).to.exist;
                },
            );
    ...
    context("Profile & User", () => {
        ...
        it("me: update username", () => {
            cy.apiPut("/me", { username: "poster_updated" }, "poster").then(
                (res) => {
                    expect(res.status).to.eq(200);
        ...
        it("me: re-login with new password", () => {
            cy.apiLogin(TEST.poster.email, "NewPass1234").then((res) => {
                expect(res.access_token).to.exist;
            });
        });
        ...
        it("public profile: returns user info", () => {
            cy.request({
                method: "GET",
                url: `/users/${Cypress.env("poster_user").id}`,
            }).then((res) => {
                expect(res.status).to.eq(200);
                expect(res.body.data.username).to.eq("poster");
            });
        ...
        it("user posts: returns posts by user", () => {
            cy.request({
                method: "GET",
                url: `/users/${Cypress.env("poster_user").id}/posts`,
            }).then((res) => {
                expect(res.status).to.eq(200);
            });
        });
    ...
    context("Categories", () => {
        ...
        it("store: user cannot create (403)", () => {
            cy.apiPost("/categories", { name: "User Category" }, "poster", {
                failOnStatusCode: false,
            }).then((res) => {
        ...
        it("destroy: admin deletes category", () => {
            cy.apiPost(
                ...
                cy.apiDelete(`/categories/${tempId}`, "superadmin").then(
                    (r2) => {
    ...
    context("Posts", () => {
        ...
        it("history: non-admin cannot view (403)", () => {
            cy.apiGet(`/posts/${postId}/history`, "poster", {
                failOnStatusCode: false,
            }).then((res) => {
                expect(res.status).to.eq(403);
            });
    ...
    context("Comments", () => {
        ...
        it("update: owner updates comment", () => {
            cy.apiPut(
                `/comments/${commentId}`,
                { body: "Updated comment from Cypress test." },
                "commenter",
            ).then((res) => {
                expect(res.status).to.eq(200);
            });
        ...
        it("update: non-owner cannot update (403)", () => {
            cy.apiPut(`/comments/${commentId}`, { body: "Hacked!" }, "voter", {
                failOnStatusCode: false,
            }).then((res) => {
                expect(res.status).to.eq(403);
            });
        ...
        it("history: admin can view comment history", () => {
            cy.apiGet(`/comments/${commentId}/history`, "superadmin").then(
                (res) => {
                    expect(res.status).to.eq(200);
                },
            );
    ...
    context("Follow", () => {
        ...
        it("follow: follow another user", () => {
            cy.apiPost(`/users/${targetId()}/follow`, {}, "commenter").then(
                (res) => {
                    expect(res.status).to.eq(201);
                },
            );
        ...
        it("follow: cannot follow self (422)", () => {
            cy.apiPost(`/users/${targetId()}/follow`, {}, "poster", {
                failOnStatusCode: false,
            }).then((res) => {
                expect(res.status).to.eq(422);
            });
        ...
        it("follow: duplicate follow (422)", () => {
            cy.apiPost(`/users/${targetId()}/follow`, {}, "commenter", {
                failOnStatusCode: false,
            }).then((res) => {
                expect(res.status).to.eq(422);
            });
        ...
        it("following: public list", () => {
            cy.request(
                `/users/${Cypress.env("commenter_user").id}/following`,
            ).then((res) => {
                expect(res.status).to.eq(200);
        ...
        it("unfollow: unfollow user", () => {
            cy.apiDelete(`/users/${targetId()}/follow`, "commenter").then(
                (res) => {
                    expect(res.status).to.eq(200);
                },
            );
        ...
        it("unfollow: cannot unfollow if not following (masih 200, no-op)", () => {
            cy.apiDelete(`/users/${targetId()}/follow`, "commenter", {
                failOnStatusCode: false,
            }).then((res) => {
                expect(res.status).to.eq(200);
            });
    ...
    context("Bookmark", () => {
        ...
        it("store: bookmark a post", () => {
            cy.apiPost("/bookmarks", { post_id: postId() }, "commenter").then(
                (res) => {
                    expect(res.status).to.eq(201);
                },
            );
        ...
        it("check: verify post is bookmarked", () => {
            cy.apiGet(`/bookmarks/${postId()}/check`, "commenter").then(
                (res) => {
                    expect(res.status).to.eq(200);
    ...
    context("Notifications", () => {
        ...
        it("read: mark notification as read", () => {
            ...
            cy.apiPatch(`/notifications/${notifId}/read`, {}, "poster").then(
                (res) => {
                    expect(res.status).to.eq(200);
                },
            );
    ...
    context("Report", () => {
        ...
        it("index: non-admin cannot list reports (403)", () => {
            cy.apiGet("/reports", "poster", { failOnStatusCode: false }).then(
                (res) => {
                    expect(res.status).to.eq(403);
                },
            );
    ...
    context("Points", () => {
        ...
        it("logs: non-admin cannot view another user points (403)", () => {
            cy.apiGet(
                `/users/${Cypress.env("poster_user").id}/points`,
                "poster",
                { failOnStatusCode: false },
            ).then((res) => {
                expect(res.status).to.eq(403);
        ...
        it("recalculate: admin recalculates points", () => {
            cy.apiPost(
                `/users/${Cypress.env("poster_user").id}/points/recalculate`,
                {},
                "superadmin",
            ).then((res) => {
                expect(res.status).to.eq(200);
